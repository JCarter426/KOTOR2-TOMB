...
void Do711UserDef(object oPartyMember,int pUserEvent)
{
    switch(pUserEvent)
    {
        case 1006://DAMAGED
        {
			// Modified to only fire if health < 50% with PC and Kreia vs the party or < 5 generally ~JC 2024-10-21
			// Moved all vision ending logic to area heartbeat script ~ JC 2024-10-27
			if( !GetLocalBoolean(OBJECT_SELF, 40) && (
				GetCurrentHitPoints(OBJECT_SELF) < 5
				|| (TSL_IsPartyVisionHostile()
					&& !TSL_IsKreiaVisionHostile()
					&& GetCurrentHitPoints(OBJECT_SELF) < GetMaxHitPoints(OBJECT_SELF) / 2)
				) ) {
				SetLocalBoolean(OBJECT_SELF, 40, TRUE);
				ChangeToStandardFaction(OBJECT_SELF, STANDARD_FACTION_NEUTRAL);
				if( TSL_ShouldVisionEnd() ) {
					SurrenderToEnemies();
